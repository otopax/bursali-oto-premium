...
export default async function KutuphaneFaultDetailPage({ params }) {
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 2, name: 'Kütüphane', item: `${SITE}/${locale}/kutuphane` },
      { '@type': 'ListItem', position: 3, name: postData.brand || 'Marka', item: `${SITE}/${locale}/kutuphane/${marka}` },
      { '@type': 'ListItem', position: 4, name: postData.model || 'Model', item: `${SITE}/${locale}/kutuphane/${marka}/${model}` },
      { '@type': 'ListItem', position: 5, name: 'Arıza Çözümleri', item: `${SITE}/${locale}/kutuphane/${marka}/${model}/arizalar` },
      { '@type': 'ListItem', position: 6, name: postData.title || kod, item: `${SITE}/${locale}/kutuphane/${marka}/${model}/arizalar/${kod}` }
    ]
  };
  ...
  const localBusinessSchema = {
    ...
    '@id': (process.env.NEXT_PUBLIC_SITE_URL || 'https://www.bursaliotoservis.com'),
    url: (process.env.NEXT_PUBLIC_SITE_URL || 'https://www.bursaliotoservis.com'),
  ...
  const serviceSchema = {
    ...
    provider: { '@id': (process.env.NEXT_PUBLIC_SITE_URL || 'https://www.bursaliotoservis.com') },
    ...
    <main style={{ minHeight: '100vh', paddingTop: '120px', paddingBottom: '5rem', background: '#09090b' }}>
      ...
      <article className="container" style={{ maxWidth: '800px', margin: '0 auto' }}>
        ...
        <nav style={{ fontSize: '0.9rem', color: 'var(--text-muted)', marginBottom: '2rem', display: 'flex', gap: '0.5rem', alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          <span>/</span>
          <a href={`/${locale}/kutuphane`} style={{ textDecoration: 'none', color: 'inherit' }}>Kütüphane</a>
          <span>/</span>
          <a href={`/${locale}/kutuphane/${marka}`} style={{ textDecoration: 'none', color: 'inherit' }}>{postData.brand}</a>
          {postData.model && (
            <>
              <span>/</span>
              <a href={`/${locale}/kutuphane/${marka}/${model}`} style={{ textDecoration: 'none', color: 'inherit' }}>{postData.model}</a>
            </>
          )}
          <span>/</span>
          <a href={`/${locale}/kutuphane/${marka}/${model}/arizalar`} style={{ textDecoration: 'none', color: 'inherit' }}>Arızalar</a>
          <span>/</span>
        ...
        {relatedFaults.length > 0 && (
          <div style={{ marginTop: '4rem' }}>
            <h2 style={{ fontSize: '1.8rem', color: 'var(--text-light)', marginBottom: '1.5rem', fontWeight: 'bold' }}>
              Diğer {postData.brand} Kütüphane Çözümleri
            </h2>
            <div className="grid">
