    ...
    <div className="container overflow-x-hidden max-w-full">
      ...
      {visibleCount < totalPhotos && (
        <div className="desktop-only" style={{ textAlign: 'center', marginTop: '2rem' }}>
          ...
          >
            Daha Fazla Fotoğraf Yükle ({totalPhotos - visibleCount} Kaldı) 👇
          </button>
        </div>
      )}
      
      {/* Mobile users just get a map link after the first 8 */}
      <div className="mobile-only" style={{ textAlign: 'center', marginTop: '1.5rem', width: '100%' }}>
      ...
      {visibleCount === totalPhotos && (
        <p className="desktop-only text-center mt-8 text-[var(--text-light)]">
