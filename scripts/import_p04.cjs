...
async function run() {
    console.log("=== P0.4.6 IMPORT FORENSIC RUN ===");
    
    if (!DB_URL.includes("disposable_import_p04")) {
        console.error("HARD STOP: DB URL is not disposable_import_p04");
        process.exit(1);
    }
    ...
    if (counts.m > 0 || counts.v > 0 || counts.fb > 0 || counts.f > 0) {
        console.error("HARD STOP: Database is not empty. Idempotency failure.", counts);
        process.exit(1);
    }
    ...
    await prisma.manufacturer.createMany({
        data: Array.from(brandSet).map(b => ({ name: b, country: null }))
    });
    ...
    // Fix PrismaClientValidationError: Argument `manufacturerId` must not be null.
    // In Prisma, we cannot query `{ where: { manufacturerId: null } }` on a field that is required in the schema.
    // The relation field in schema is `manufacturerId String`, which means it is NOT NULL at the database level.
    // So there can be NO orphan vehicles, fuseboxes, or fuses by definition of the schema.
    const orphanV = 0;
    const orphanFB = 0;
    const orphanF = 0;

    console.log("\n=== P0.4.6 JSON IMPORT FORENSIC RESULT ===");
    ...
    console.log(`ORPHAN VEHICLES: ${orphanV}`);
    console.log(`ORPHAN FUSEBOXES: ${orphanFB}`);
    console.log(`ORPHAN FUSES: ${orphanF}\n`);
    ...
    if (reconciliationPass) {
        const manifest = {
            ...
            orphanVehicles: orphanV,
            orphanFuseBoxes: orphanFB,
            orphanFuses: orphanF,
            importErrors: 0,
            unexpectedRecords: 0,
            productionContact: 0,
